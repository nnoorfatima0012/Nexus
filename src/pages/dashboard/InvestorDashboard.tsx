...
import { Entrepreneur, NexusCollaboration } from "../../types";
import { getEntrepreneurs } from "../../services/userService";
...
export const InvestorDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        const [entrepreneursData, collaborationsData] = await Promise.all([
          getEntrepreneurs(),
          getMyCollaborations(),
        ]);

        setEntrepreneurs(entrepreneursData.users || []);
  ...
  const sentPendingRequests = collaborations.filter(
    (collaboration) =>
      collaboration.sender._id === user.id && collaboration.status === "pending",
  );
  ...
  const industries = Array.from(
    new Set(
      entrepreneurs
        .map((entrepreneur) => entrepreneur.industry)
        .filter(Boolean),
    ),
  );
        ...
        <CardHeader className="flex justify-between items-center">
          ...
          {sentPendingRequests.length > 0 && (
            <Badge variant="warning">
              {sentPendingRequests.length} pending sent
            </Badge>
          )}
        </CardHeader>
